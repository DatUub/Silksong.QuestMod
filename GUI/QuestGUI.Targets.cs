...
namespace QuestMod
{
    public partial class QuestGUI
    {
        private void DrawCompletionTab()
        {
            ...
            for (int i = 0; i < QuestRegistry.Categories.Length; i++)
            {
                var style = (i == categoryTab)
                    ? GUI.skin.GetStyle("ToolbarButtonActive")
                    : GUI.skin.GetStyle("ToolbarButton");
                if (GUILayout.Button(QuestRegistry.Categories[i], style))
                    categoryTab = i;
            }
            ...
            GUILayout.BeginHorizontal();
            GUILayout.Label(new GUIContent("Multiplier:", "Scale all targets in this category (0 to 1)"), GUILayout.Width(68));
            float prevSlider = multiplier;
            multiplier = GUILayout.HorizontalSlider(multiplier, 0f, 1f, GUILayout.Width(150));
            // 0.1 snap matches the display.
            multiplier = Mathf.Round(multiplier * 10f) / 10f;

            // Resync only when the slider moved or typing gets clobbered.
            if (!Mathf.Approximately(multiplier, prevSlider))
                multiplierText = multiplier.ToString("0.00");

            multiplierText = GUILayout.TextField(multiplierText, GUILayout.Width(40));

            if (GUILayout.Button("Apply", GUILayout.Width(48)))
            {
                if (float.TryParse(multiplierText, System.Globalization.NumberStyles.Float,
                        System.Globalization.CultureInfo.InvariantCulture, out float parsed))
                {
                    parsed = Mathf.Clamp(parsed, 0f, 1f);
                    multiplier = parsed;
                    multiplierText = parsed.ToString("0.00");
