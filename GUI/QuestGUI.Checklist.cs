    ...
    public partial class QuestGUI
    {
        private void DrawChecklistTab()
        {
            ...
            foreach (var kvp in QuestRegistry.ChecklistQuests)
            {
                ...
                GUILayout.Label($"{questName}{seqTag}  ({doneCount}/{labels.Length})", QuestGUISkin.SectionHeader);
                ...
                for (int i = 0; i < labels.Length && i < status.Length; i++)
                {
                    ...
                    // Lock unmet stages outside Pure.
                    if (isSequential && canToggle && !QuestModPlugin.IsPureWishes
