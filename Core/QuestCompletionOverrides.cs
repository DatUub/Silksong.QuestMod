...
namespace QuestMod
{
    ...
    public static class QuestCompletionOverrides
    {
        ...
        // Restore counts before dropping cache -- FullQuestBase is global,
        // slot A's edits leak into slot B otherwise.
        public static void ResetSlotState()
        {
            if (cachedQuests != null && countField != null)
            {
                ...
                foreach (var quest in cachedQuests)
                {
                    ...
                    var name = quest.name ?? "";
                    if (!originalCounts.TryGetValue(name, out var orig)) continue;
                    var arr = GetTargetsArray(quest);
                    if (arr == null) continue;
                    int n = System.Math.Min(arr.Length, orig.Length);
                    for (int i = 0; i < n; i++)
                    {
                        var target = arr.GetValue(i);
                        if (target == null) continue;
                        countField.SetValue(target, orig[i]);
                        if (target.GetType().IsValueType) arr.SetValue(target, i);
                        restored++;
                    }
                }
                QuestModPlugin.Log?.LogInfo($"QuestCompletionOverrides.ResetSlotState: restored {restored} target counts on title return");
            }
            cachedQuests = null;
        ...
        {
            if (!IsInitialized) return;
            var found = Resources.FindObjectsOfTypeAll<FullQuestBase>();
            // Empty = Resources not ready; retry next call rather than cache empty.
            if (found == null || found.Length == 0)
            {
                QuestModPlugin.Log?.LogDebug("CacheQuests: FullQuestBase array empty, will retry next call");
                return;
            }
            cachedQuests = found;
        ...
        // Set without persisting. Caller re-applies each scene load.
        ...
        public static void ApplySavedOverrides()
        {
            ...
            // Presets apply after slider edits. User edits win.
        ...
        public static void ToggleChecklistTarget(string questName, int index, bool done)
        {
            ...
            // Sequential is monotonic: tick N zeroes 0..N-1, untick sets N+1.. to 1.
            if (QuestRegistry.SequentialQuests.Contains(questName))
            {
                var labels = QuestRegistry.ChecklistQuests[questName];
                if (done)
                {
                    for (int i = 0; i < index; i++)
                        SetTargetCount(questName, i, 0);
                }
                else
                {
