...
{
    /// <summary>
    /// Customizes the Great Gourmand quest item (Courier's Rasher).
    ///
    /// Verified via Resources decompile + Apollo DLL inspection:
    ///   - The Rasher is a DeliveryQuestItem (base class extends CollectableItem).
    ///   - DeliveryQuestItemStandalone is a *subclass* used for one-item-completes
    ///     deliveries; it adds targetCount + invItemAppendDesc but has NO timer fields.
    ///   - The decay duration is the `totalTimer` (float) field on the BASE
    ///     DeliveryQuestItem class.
    ///   - The decay tick is the static method DeliveryQuestItem.TakeHit(int hits)
    ///     which iterates active items and calls TakeHitForItem on each.
    ///   - The "8 segments" UI is purely a HUD presentation
    ///     (DeliveryHudIcon.maxItemCount), NOT a quest-data field, so we don't
    ///     expose a segment-count config knob here. Segment count is governed by
    ///     each delivery item's prefab HUD setup; modders who want to change it
    ///     would have to mutate the spawned HUD instance.
    ///
    /// Two knobs (config in QuestModPlugin):
    ///   - GourmandStopDecay - when true, the static TakeHit prefix returns false
    ///     so the original method is skipped, halting all delivery decay this frame.
    ///   - GourmandDecaySeconds - when set (and != default 47), overrides
    ///     totalTimer on every spawned DeliveryQuestItem instance on scene load.
    /// </summary>
    public static class GourmandTimerPatch
    {
        ...
        private static void EnsureResolved()
        {
            ...
            if (!harmonyApplied && takeHitIntMethod != null)
            {
                try
                {
                    var harmony = new Harmony("com.silkmod.questmod.gourmand-timer");
