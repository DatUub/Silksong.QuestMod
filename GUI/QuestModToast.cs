...
namespace QuestMod
{
    // I-2: lightweight on screen notification queue. Renders short messages
    // in the upper right corner of the screen for a few seconds, fading out
    // automatically. Used to surface mod actions that the player otherwise
    // would only see in the BepInEx log (auto accept counts, mode changes).
    //
    // Always rendered even when the F9 panel is closed, so the player sees
        ...
        public static void DrawAll()
        {
            lock (_lock)
            {
                ...
                foreach (var e in _entries)
                {
                    ...
                    // Fade based on remaining time so the toast eases out.
