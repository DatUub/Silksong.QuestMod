    ...
    public static class SilkSoulOverrides
    {
        ...
        private static void TryResolveEntries()
        {
            ...
            foreach (var field in AccessTools.GetDeclaredFields(groupType))
            {
                ...
                // First float is the canonical point value.
                foreach (var ef in AccessTools.GetDeclaredFields(elemType))
                {
                    if (questField == null &&
                        (ef.FieldType.Name.Contains("Quest") || ef.FieldType.Name.Contains("FullQuestBase")))
                    {
                        questField = ef;
                    }
                    else if (valueField == null && ef.FieldType == typeof(float))
                    {
                        valueField = ef;
                    }
                }
                // Fall back to first int.
                if (valueField == null)
                {
                    foreach (var ef in AccessTools.GetDeclaredFields(elemType))
                    {
                        if (ef.FieldType == typeof(int)) { valueField = ef; break; }
                    }
                }
        ...
        public static bool SetPointValue(int entryIndex, float value)
        {
            ...
            // Negative breaks sum logic; huge values overflow int cast.
            ...
            {
                int truncated = UnityEngine.Mathf.RoundToInt(value);
                typedValue = truncated;
                value = truncated; // keep override store in sync with what game saw
            }
        ...
        {
            // Cap at 100; negative never unlocks.
            if (value < 0) value = 0;
        ...
        {
            // Restore struct before nulling cache or save A leaks into save B.
            try
            {
                if (cachedGroup != null && f_target != null)
                    ReflectionCache.Write(cachedGroup, f_target, QuestRegistry.DefaultThreshold);
                if (HasEntries)
                {
                    for (int i = 0; i < cachedEntries.Length; i++)
                    {
                        ...
                        var questObj = f_entryQuest.GetValue(entry) as UnityEngine.Object;
                        if (questObj == null) continue;
                        if (!QuestRegistry.SilkSoulPointValues.TryGetValue(questObj.name, out float def))
                            continue;
                        object typedValue = f_entryValue.FieldType == typeof(float)
                            ? (object)def
                            : (object)UnityEngine.Mathf.RoundToInt(def);
                        ReflectionCache.WriteToArray(cachedEntries, i, entry, f_entryValue, typedValue);
                    }
                }
            }
            ...
            {
                QuestModPlugin.Log?.LogWarning($"SilkSoul.Reset restore failed: {ex.Message}");
            }
            cachedGroup = null;
            cachedEntries = null;
            resolved = false;
