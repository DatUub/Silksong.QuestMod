...
namespace QuestMod
{
    ...
    {
        // Quest IDs that belong on the Delivery tab. We pull these from the
        // "Delivery" category in QuestCapabilities.json plus the Great Gourmand
        // wish (which is technically a Gather quest but uses delivery items).
        private const string GourmandQuestId = "Great Gourmand";

        private Vector2 deliveryScroll;
        private string gourmandSegmentInput = "";
        ...
        private void DrawGourmandTimerSection()
        {
            ...
            // Segment count
            int seg = QuestModPlugin.GourmandSegmentCount.Value;
            if (gourmandSegmentInput == "") gourmandSegmentInput = seg.ToString();

            GUILayout.BeginHorizontal();
            GUILayout.Label(new GUIContent("Segments:", "Number of durability segments (default 8)."), GUILayout.Width(90));
            if (GUILayout.Button("−", GUILayout.Width(22)))
            {
                int v = Mathf.Max(1, seg - 1);
                QuestModPlugin.GourmandSegmentCount.Value = v;
                gourmandSegmentInput = v.ToString();
            }
            gourmandSegmentInput = GUILayout.TextField(gourmandSegmentInput, GUILayout.Width(40));
            if (GUILayout.Button("+", GUILayout.Width(22)))
            {
                int v = Mathf.Min(32, seg + 1);
                QuestModPlugin.GourmandSegmentCount.Value = v;
                gourmandSegmentInput = v.ToString();
            }
            if (GUILayout.Button("Set", GUILayout.Width(36)))
            {
                if (int.TryParse(gourmandSegmentInput, out int v))
                    QuestModPlugin.GourmandSegmentCount.Value = Mathf.Clamp(v, 1, 32);
            }
            if (GUILayout.Button("Reset", GUILayout.Width(48)))
            {
                QuestModPlugin.GourmandSegmentCount.Value = 8;
                gourmandSegmentInput = "8";
            }
            if (seg != 8)
                GUILayout.Label("(default: 8)", GUILayout.Width(80));
            GUILayout.FlexibleSpace();
            GUILayout.EndHorizontal();

            // Decay seconds slider
            ...
            float total = decay * seg;
            GUILayout.Label($"  Total carry budget: {total:0.#}s ({seg} × {decay:0.#}s)");
        ...
        private void DrawDeliveryQuestList()
        {
            ...
            foreach (var name in deliveryNames)
            {
                ...
                {
                    // Quest not in runtime/resource list yet — synthesize a stub row
                    quest = new QuestInfo
                    {
                        Name = name,
